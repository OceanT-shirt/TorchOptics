...
import tensorflowlens.ray_tracing as rt
import tensorflowlens.lens_modeling as lm
import image_ops
from utils.w2rgb import wavelength_to_rgb
...
class OpticsSimulator(tf.keras.Model):
    ...
    def initialize(self):
        ...
        # Lens specifications
        self.hfov = tf.Variable(initial_value=np.radians(np.array(self.initial_lens['hfov'][-1]).reshape([1,])),
                                name='hfov', dtype=tf.float32,
                                trainable=False)
        self.epd = tf.Variable(initial_value=np.array(self.initial_lens['epd']),
                                    name='epd', dtype=tf.float32,
                                    trainable=False)
        ...
        self.lensR = lm.Lens(self.structure, *[tf.constant(self.initial_lens[key]) for key in ['c', 't', 'nd', 'v']])

        # The actual optimized variables are c, t and g
        self.c, self.t, self.g = lm.get_normalized_lens_variables(
            self.lensR, self.trainable_vars, self.add_bfl, self.scale_factor)
    ...
    def initialize2(self):
         # Lens structure
        self.structure = lm.Structure(
            stop_idx=self.StopIndex,
            sequence=self.Sequence
        )

        # Lens specifications
        self.hfov = tf.Variable(initial_value=np.radians(self.HFOV[-1].reshape([1,])),
                                name='hfov', dtype=tf.float32,
                                trainable=False)
        self.epd = tf.Variable(initial_value=self.EPD, 
                                    name='epd', dtype=tf.float32,
                                    trainable=False)
        ...
        # Lens variables - with normalized forms
        self.lensR = lm.Lens(self.structure, *[key for key in [self.Curv, self.Thickness, self.Nref, self.Abbe]])
    ...
    def apply_optics_model(self, radiance, field_lim, max_value=255.):
        """
            Simulate the aberrations on the input_tensor radiance image
        ...
        if self.apply_distortion and self.distortion_by_warping:
            ...
            ax = h.add_subplot()
            n = 64
            ax.plot(x_img.numpy()[0:n], y_img.numpy()[0:n], 'b.')
            ax.plot(x_img_dist.numpy()[0:n], y_img_dist.numpy()[0:n], 'r.')
            plt.show()
...
class RaytracedOptics(OpticsSimulator):
    ...
    def __init__(self,
                 ...
                 glass_weight=.01,
                 glass_catalog_path='./selected_ohara_glass.csv',
                 loss_multiplier=1,
                 **kwargs
                 ):
        super(RaytracedOptics, self).__init__(
                                        initial_lens_path=InitialLensPath,
                                        StopIndex=StopIndex,
                                        Sequence=Sequence, 
                                        HFOV=HFOV,
                                        EPD=EPD, 
    ...
    def compute_loss_out(self, rt_outputs):
        ...
        # compute penalty Q
        numSequence = len(self.Sequence[0])
        Q = (tf.reduce_sum(stacks['theta_norm'],0)+tf.reduce_sum(stacks['theta_prime_norm'],0)+tf.reduce_sum(stacks['z_RELU'],0))/numSequence
        # replace nan with zero
        Q = tf.where(tf.math.is_nan(Q), tf.zeros_like(Q),Q)
        sumQ = tf.reduce_sum(Q)
        lq = 0.2
        Lu = rms + lq*sumQ
        self.loss_dict = {'loss_unsup': Lu}
    ...
    def sample_optics_model(self):
        ...
        specs = self.specs
        lens = self.lens
        x, y, *_ = self.do_ray_tracing(lens)
        ...
        if self.apply_relative_illumination:
            mean_wavelengths = np.mean(self.wavelengths)
    ...
    def GetTraceResult(self):
        ...
        # run ray tracing
        lens = self.lensR     # original scale system
        x, y, ray_ok = self.do_ray_tracing(lens, should_log=False)
        # convert tf into double
        ray_ok = ray_ok.numpy()

        # loss function
        loss_unsup = self.loss_dict['loss_unsup']
        return x, y, ray_ok, loss_unsup

    def ShowTraceResult(self, x, y, ray_ok, loss_unsup):
        
        h = plt.figure()
        h.suptitle('Unsupervised Loss Function Output:\n' + str(loss_unsup.numpy()), fontsize=12)
        ax = h.add_subplot()
        for f in range(0,x.shape[1]):
            for w in range(0,x.shape[3]):
                ...
                for p in range(0,x.shape[2]):
                    if ray_ok[0,f,p,w]:
                        ax.plot(x[0,f,p,w], y[0,f,p,w], '.', color=PltColor, markersize=4)
