...
class RayTracer:
    ...
    def __init__(self, mode='skew_random', n_rays=(8, 8), rel_fields=(0., 0.707, 1.), vig_fn=None,
        ...
        if self.mode == 'skew_random':
            ...
            assert len(n_rays) == 2
            self.pupil_span = lambda tensor: circle(tensor, *n_rays)
        else:
...
def circle(tensor, n_r, n_theta):
    """
        circular distrubution
    """
    n_rays = n_r * n_theta
    r = tf.constant(np.linspace(0, 1.0, n_r, endpoint=False, dtype=np.float32))[None, :, None]
    theta = tf.constant(np.linspace(0, 2*np.pi, n_theta, endpoint=False, dtype=np.float32))[None, None, :]
    x = r * tf.cos(theta)
    y = r * tf.sin(theta)
...
def trace_skew(x, y, z, cx, cy, c, t, mu, mask, aggregate=False, allow_backward_rays=True):
    ...
    for k in range(len(t)):
        ...
        if aggregate:
            zRELU = tf.identity(z)
            zRELU = tf.where(zRELU<=0, tf.zeros_like(zRELU), zRELU)
            eps = 1e-7
            cos_clamped = tf.clip_by_value(tf.sqrt(cos2_theta), -1.0+eps, 1.0-eps)
            cos_prime_clamped = tf.clip_by_value(tf.sqrt(cos2_prime), -1.0+eps, 1.0-eps)
            theta = tf.math.acos(cos_clamped)
            theta_prime = tf.math.acos(cos_prime_clamped)
            # normalise
            theta = theta / (1/2*math.pi)
            theta_prime = theta_prime / (1/2*math.pi)
            # ray_failure
            theta = tf.where(~ray_ok, tf.ones_like(theta), theta)
            theta_prime = tf.where(~ray_ok, tf.ones_like(theta_prime), theta_prime)
            stacks['z_RELU'].append(tf.broadcast_to(zRELU, (*x.shape[:3], mu[0].shape[-1])))
            stacks['theta_norm'].append(tf.broadcast_to(theta, (*x.shape[:3], mu[0].shape[-1])))
            stacks['theta_prime_norm'].append(tf.broadcast_to(theta_prime, (*x.shape[:3], mu[0].shape[-1])))
...
def compute_rms2d(x, y, ray_ok):
    """
        ...
        Using cost function from the article 2019 "Extrapolating from lens design databases using
        deep learning"
    ...
    for f in range(0,numF):
        ...
        for w in range(0,numW):
            ...
            # xy = np.hstack((tf.reshape(x[0,f,:,w], [numP,1]), tf.reshape(y[0,f,:,w], [numP,1])))
            # centroid = list(Polygon(xy).centroid.coords)
            # YCent = centroid[0][1]
            YCent = tf.reduce_mean(y[0,f,:,w])
            YCentCum = YCentCum + YCent

        ymean = YCentCum/numW
        RMSCum = RMSCum + tf.sqrt(tf.reduce_sum((y[0,f,:,:][ray_ok[0,f,:,:]] - ymean)**2)/(numP*numW))
