...
def trace_skew(x, y, z, cx, cy, c, t, mu, mask, aggregate=False, allow_backward_rays=True):
    ...
    """
    stacks = {k: [] for k in ('x', 'y', 'z', 'sin', 'sin_prime', 'cos', 'cos_prime', 'cos2', 'cos2_prime')}
    ...
    for k in range(len(t)):
        ...
        if aggregate:
            ...
            stacks['sin'].append(tf.broadcast_to(tf.sqrt(1-cos2_theta), (*x.shape[:3], mu[0].shape[-1])))
            stacks['sin_prime'].append(tf.broadcast_to(tf.sqrt(1-cos2_prime), (*x.shape[:3], mu[0].shape[-1])))
            # stacks['cos'].append(tf.broadcast_to(cos_theta, (*x.shape[:3], mu[0].shape[-1])))
            # stacks['cos_prime'].append(tf.broadcast_to(cos_prime, (*x.shape[:3], mu[0].shape[-1])))
