...
class OpticsSimulator(nn.Module):
    ...
    """
    Thanks Sasaki-san for the following code
    """
...
class RaytracedOptics(OpticsSimulator):
    ...
    def __init__(self,
                 ...
                 glass_weight=.01,
                 glass_catalog_path='../glass/selected_ohara_glass.csv',
                 loss_multiplier=1,
                 **kwargs
                 ):
    ...
    #         'spot_size': torch.mean(rt.compute_rms2d(x, y, ray_ok)),
    ...
    # def find_effective_aperture(self, lens=None):

    #     specs = self.specs
    #     lens = lens or self.lens

    #     # check stop_index = 1 and hfov = 0
    #     self.hfov=torch.tensor([0.])
    #     self.stop_index = 1
    #     # trace 1D ray bandle along y axis and # save intersections through surfaces
    #     ray_tracer = rt.RayTracer(**rt_params)
    #     rt_outputs = ray_tracer.trace_rays(specs, lens, aggregate=True)
    #     x, y, *_, ray_ok, ray_backward, stacks = rt_outputs
    #     # compute maximum ray height for each surfaces

    #     return effective_stop_index
