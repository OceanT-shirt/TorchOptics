...
class RayTracer:
    ...
    def trace_rays(self, specs, lens, use_vig=True, aggregate=False, xy=None, up_to_stop=False):
        ...
        n = torch.transpose(n, dim0=0, dim1=2)
        ...
        if self.n_ray_aiming_iter > 0 and up_to_stop is False:
            ...
            xp_rel, yp_rel = [torch.autograd.Variable(torch.clamp(item, -2, 2), requires_grad=False) for item in
        ...
        u = (specs.hfov[:, None] * torch.tensor(self.rel_fields, dtype=tf.float32)[None, :])[..., None, None]
        cy = torch.sin(u)
        cx = torch.reshape(torch.zeros(1), (1, 1, 1, 1))

        # Adjust dimensions
        c = torch.reshape(lens.c, (lens.c.shape[0], 1, 1, 1, -1))
        t = torch.reshape(lens.t, (lens.t.shape[0], 1, 1, 1, -1))
        mu = n[..., :-1] / n[..., 1:]
        mask = torch.reshape(lens.structure.mask, (lens.c.shape[0], 1, 1, 1, -1))
