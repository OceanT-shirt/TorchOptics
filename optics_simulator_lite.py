...
class OpticsSimulator(nn.Module):
    ...
    def initialize(self):
        ...
        # Lens specifications
        self.hfov = torch.nn.Parameter(torch.tensor(np.radians(self._hfov[-1].reshape([1, ]))),
                                       requires_grad=False)
        self.epd = torch.nn.Parameter(torch.tensor(self._epd),
                                      requires_grad=False)
        ...
        self.lensR = lm.Lens(self.structure, *[key for key in [self._curvature, self._thickness, self._n_refractive, self._abbe_number]])
